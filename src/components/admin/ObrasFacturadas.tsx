...
function ObrasTerminadas() {
  ...
  useEffect(() => {
    const fetchObras = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, "obras_facturadas"));
  ...
  if (loading)
    return <p className="text-center mt-6">Cargando obras facturadas...</p>;
  ...
  return (
    <div className="max-w-4xl mx-auto mt-8 space-y-4 px-4 ">
      ...
      {obras.length === 0 && (
        <p className="text-center text-gray-600">No hay obras facturadas.</p>
      )}
    </div>
  );
}
